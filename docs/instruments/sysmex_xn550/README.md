...
**Status: field-verified reference instrument.** Phase 1 (G1 raw capture: listener transport, message assembly, exact raw persistence) and Phase 2 (the dedicated XN-550 parser and envelope classification written back to each raw row) are implemented and **disabled by default**. There is no result visibility, no observation persistence and no clinical integration (§6). **OD-XN-3 is approved** by the project owner and the laboratory ([`M9.2_IMPLEMENTATION_CONTRACT.md`](M9.2_IMPLEMENTATION_CONTRACT.md) §0.3, §19.6.8). G2 may be implemented and tested, but nothing of G2 is implemented yet. Production XN-550 stays at `ingestion_stage: "raw_only"` until the 14-day G1 soak exit criteria pass (§19.7).
...
| Results API / frontend | **None yet.** OD-XN-3 is **approved** (§0.3), so G2 may be implemented and tested against the closed contract. Production stays `raw_only` until the 14-day G1 soak exit criteria pass (§19.7). The closed contract covers:<br>• tables, constraints and indexes (§10)<br>• provenance offsets and text layers (§11.1)<br>• the identity boundary (§12.3)<br>• duplicate handling and the per-instrument lock (§13, §14.2)<br>• the frozen read-only API and UI (Appendices B, C)<br>The approved OD-XN-3 answers are in §19.6.8. Not implemented yet |
