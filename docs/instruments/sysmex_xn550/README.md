...
> - **Implementation:** listener mode is implemented for **G1 and G2** (raw capture, envelope classification and unlinked observations), restricted in code to this approved scope, and **disabled by default** (§6). Production would run G1 (`raw_only`) during the soak before G2 is enabled there.
