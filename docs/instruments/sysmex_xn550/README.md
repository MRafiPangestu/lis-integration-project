...
**Status: field-verified reference instrument.** Phase 1 (G1 raw capture: listener transport, message assembly, exact raw persistence) and Phase 2 (the dedicated XN-550 parser and envelope classification written back to each raw row) are implemented and **disabled by default**. There is no result visibility, no observation persistence and no clinical integration (§6). The G2 contract is closed for implementation: schema, duplicate handling, identity boundary and the read-only API/UI surface are pinned. The OD-XN-3 decision package, with sub-decisions OD-XN-3.1 to OD-XN-3.5, is in [`M9.2_IMPLEMENTATION_CONTRACT.md`](M9.2_IMPLEMENTATION_CONTRACT.md) §19.6. **OD-XN-3 is still open**, and nothing of G2 is implemented.
...
| Results API / frontend | **None.** G2 observations are not implemented, because OD-XN-3 is open. The G2 contract is closed for implementation:<br>• tables, constraints and indexes (§10)<br>• provenance offsets and text layers (§11.1)<br>• the identity boundary (§12.3)<br>• duplicate handling and the per-instrument lock (§13, §14.2)<br>• the frozen read-only API and UI (Appendices B, C)<br>The OD-XN-3 decision table is §19.6.8. Design only |
...
| [`M9.2_IMPLEMENTATION_CONTRACT.md`](M9.2_IMPLEMENTATION_CONTRACT.md) | Design-only implementation contract (2026-09-17): listener transport, CR-record message assembly, raw-first persistence, field confidence levels, unlinked-observation layer, identity and duplicate strategy, tests and acceptance criteria. Its "M9.2" label is **not** the `docs/07` BC-5150 deduplication milestone (see its §0.1). Implementation notes: §19.4 (Phase 1), §19.5 (Phase 2). G2 readiness review, contract closure and the OD-XN-3 decision package: §19.6 |
