...
**Status: field-verified reference instrument.** Phase 1 (G1 raw capture: listener transport, message assembly, exact raw persistence) and Phase 2 (the dedicated XN-550 parser and envelope classification written back to each raw row) are implemented and **disabled by default**. There is no result visibility, no observation persistence and no clinical integration (§6). A G2 readiness review and the OD-XN-3 decision request are prepared in [`M9.2_IMPLEMENTATION_CONTRACT.md`](M9.2_IMPLEMENTATION_CONTRACT.md) §19.6; **OD-XN-3 is still open**.
...
| Results API / frontend | **None.** G2 observations are not implemented, because OD-XN-3 is open. Contract §19.6 holds the G2 readiness review — schema, idempotency, identity boundary, the derived read-only API/UI surface and acceptance-criteria readiness — and the OD-XN-3 decision request. Design only |
...
| [`M9.2_IMPLEMENTATION_CONTRACT.md`](M9.2_IMPLEMENTATION_CONTRACT.md) | Design-only implementation contract (2026-09-17): listener transport, CR-record message assembly, raw-first persistence, field confidence levels, unlinked-observation layer, identity and duplicate strategy, tests and acceptance criteria. Its "M9.2" label is **not** the `docs/07` BC-5150 deduplication milestone (see its §0.1). Implementation notes: §19.4 (Phase 1), §19.5 (Phase 2). G2 readiness review and OD-XN-3 decision request: §19.6 |
