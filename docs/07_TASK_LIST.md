...
> - Still open: genuine rerun, ACK-timeout retry / NAK, query/pull, automatic queue flush / send-all, disconnect on abrupt power loss or other causes, and all non-patient message categories (QC, calibration, maintenance, startup — not observed at the LIS). Reconnect behaviour is only **partially verified**; disconnect on a normal shutdown and reconnect after boot were **observed once** (POWER-CYCLE-01), with no application payload in the observed window; an explicit five-record multi-select transmit produced five messages (MULTI-SELECT-01, one observation, not counted as corpus). Final Day-18 status: `docs/instruments/sysmex_xn550/VALIDATION_2026-09-17.md` §20.
>
...
- [x] Add migration verification to the test suite: provision a scratch database through the migration chain and assert the result matches `Base.metadata` — **E1 evidence: `backend/tests/test_migration_chain.py` (commit `b7c3d0e`)** (Acceptance-Criteria item)
- [x] Add a provisioning section to `04_DATABASE_DESIGN.md` documenting the fresh-install path and the legacy-upgrade path — **committed (commit `ffb81f9`, §29 Provisioning & Migration Lifecycle)** (Acceptance-Criteria item)
...
### M9.1b — Audit Attribution — **COMPLETE**

`02_PRD.md` NFR-08 requires workflow audit information to include the identity of the user who performed the activity; `03_SYSTEM_DESIGN.md` §12 lists final-run selection among the activities to be traceable. No user identity was recorded on any workflow mutation before this milestone.

**Completed, built on the frozen design in `docs/M9.1b_AUDIT_ATTRIBUTION_DESIGN.md` (Owner Decision Record: OD-B1–OD-B7).** One append-only `audit_events` table (migration `28aa370f5dbe`, `down_revision = 27e00bcff992`) — no actor columns added to any existing domain table (OD-B1). Explicit `actor: User` propagation, no contextvar/thread-local/request-global identity, threaded through the five `TestRunService` methods and all eight attributed routes. Audit rows are staged in the same transaction as the business mutation they record, before the existing commit — no new commit, no new transaction boundary. `sync-simrs` produces exactly two audit rows (`DELIVERY_STARTED` then `DELIVERY_DELIVERED`/`DELIVERY_FAILED`) without any special-casing, verified with an independent-connection test proving the outbound SIMRS call never runs inside an open transaction. User-management actions are attributed (`USER_CREATED`, `USER_DISABLED`, `USER_REACTIVATED`, `PASSWORD_CHANGED` — OD-B2); login is excluded, `last_login_at` remains the sufficient record. Only successful, committed transitions are recorded — a no-op status update or a denied (401/403) request writes zero rows (OD-B3). Instrument ingestion writes zero audit rows and introduces no synthetic actor — verified with a dedicated regression test (`backend/tests/test_ingestion.py`). `id_user` is an authoritative FK to `users.id_user` with `ON DELETE RESTRICT` (verified: deleting a user with audit history is rejected; deactivation preserves it), plus `actor_username`/`actor_role` snapshots (OD-B5). Immutability is application-level only — no route updates or deletes `audit_events`, no DB trigger (OD-B4). Retention is indefinite (OD-B6); no correlation/request ID was added (OD-B7).

**Acceptance Criteria Evidenced.** All three of this milestone's own criteria are met: (1) the acting user is recorded on finalize/unfinalize/delivery/SIMRS sync — **E1:** `backend/tests/test_test_run_service_audit.py`, `backend/tests/api/test_test_run_audit_api.py`, `backend/tests/api/test_sync_simrs_audit_api.py`; (2) a schema change with its migration — **E1:** `backend/tests/test_migration_chain.py` (14 passed, measured counts), **E2:** `28aa370f5dbe_m9_1b_add_audit_events_table.py`; (3) tests asserting attribution is recorded and is not spoofable — **E1:** `test_actor_cannot_be_spoofed_via_body_query_or_header` (`test_test_run_audit_api.py`) and `test_actor_cannot_be_spoofed_on_user_management_endpoints` (`backend/tests/api/test_user_management_audit_api.py`), both proving a body/query/header-supplied identity is ignored and the recorded actor always matches the authenticated caller. Full backend suite: **334 passed.** DEV (`lis_marina_permata_dev`) migrated to `28aa370f5dbe` and a full 15-step end-to-end acceptance pass (login → create → clinical mutation → disable → revocation → reactivate → password change → sync-simrs) succeeded, using dedicated verification accounts and fixtures — the real pre-existing `admin`/`analyst1` accounts and accumulated clinical data were not touched. Stable PoC `lis_marina_permata` was never connected to for mutation and remains at its original 9 tables.

- [x] Record the acting user on: finalize, unfinalize, delivery state transitions, SIMRS sync
- [x] Schema change to carry the attribution, with its migration
- [x] Tests asserting attribution is recorded and is not spoofable by the caller

> **OD-1 is resolved to Option B**, so role no longer distinguishes who performed a clinical workflow action; attribution was the only remaining control satisfying NFR-08. M9.1b is now complete, closing that gap.

**Not delivered by M9.1b, deliberately:** a true multi-connection concurrency test for `with_for_update(nowait=True)` (existing behaviour preserved, not newly exercised); the pre-existing M9.1a "last active ADMIN" guard remains structurally unreachable via the API (unchanged, not a M9.1b gap); M9.2, M9.3b, M10.1, OD-S3 (SIMRS inbound authentication), OD-S6 (TLS/HTTPS), instrument connection-history remediation, and failed/denied-attempt security logging are all explicitly out of scope and untouched.
...
| M9.1b — Audit Attribution | ✅ COMPLETE | Append-only `audit_events` (migration `28aa370f5dbe`); explicit `actor: User` propagation, same-transaction writes; sync-SIMRS two-event attribution; user-management attribution (OD-B2); ingestion excluded, zero synthetic actor. 334 backend tests + 14 migration tests passed; DEV migrated to head, 15/15 E2E steps passed; stable PoC untouched. Gate in every posture; NFR-08 |
