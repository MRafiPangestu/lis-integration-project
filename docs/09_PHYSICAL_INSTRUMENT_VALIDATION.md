...
|---|---|---|---|---|---|---|---|---|---|
| T-CONN-01-03 | Sysmex XN-550 | Role / IP / port / protocol / framing | P2 | **FIELD TESTED** (2026-09-16, 2026-09-17) — endpoint, role, protocol and framing established for the tested configuration; inbound tested on **port 5001 only**; reconnect after physical link interruption **PARTIALLY VERIFIED** (cause confounded); disconnect on normal shutdown (instrument-side RST) and reconnect after boot **VERIFIED for one observation** (POWER-CYCLE-01). See `docs/instruments/sysmex_xn550/VALIDATION_2026-09-16.md` and `VALIDATION_2026-09-17.md` | Endpoint + protocol confirmation | No | No | No | **Yes** |
| T-CORPUS-01-03 | Sysmex XN-550 | Raw message corpus | P2 | **IN PROGRESS** — **count criterion MET:** 20 messages (1 + 7 + 4 + 8 across 2026-09-15/16/17, incl. Run03), 16 distinct patient-result payloads, 10 distinct structures, plus 1 uncontrolled message of unknown class. **Category criterion NOT MET:** patient results only; **zero** QC, calibration, maintenance or startup captures (a startup background check seen on the instrument screen during POWER-CYCLE-01 produced no LIS message and is not counted). Gate **not** met. See `docs/instruments/sysmex_xn550/VALIDATION_2026-09-17.md` §16 | ≥20 raw messages across categories | Yes | Yes | Yes | **Yes** |
