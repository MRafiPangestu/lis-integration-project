...
Scope: upgrade / provisioning only.

* This is NOT a downgrade test. ``b1f9dbe772fa.downgrade()`` is a known separate
  defect (unnamed-FK drops); R0 downgrade was validated separately in scratch.
...
def test_fresh_install_constraint_and_index_counts(scratch_at_head: str) -> None:
    ...
    assert snap.count("p") == 11
    assert snap.count("u") == 5
    assert snap.count("f") == 10
    assert snap.count("c") == 0
    assert len(snap.not_null_columns()) == 41
    assert len(snap.index_names()) == 21
